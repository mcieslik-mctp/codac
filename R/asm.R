...
.runInchworm <- function(bun, ann) {
    ...
    if (err==0 && length(out)>0) {
        ctgs <- data.table(
            ...
            ctg.len = lengths(out))
        ctgs <- ctgs[(ctg.cov >= ann$par$asm.ctg.cov) | (ctg.len >= ann$par$asm.ctg.len)]
    } else {
        ctgs <- ASM.EMPTY[["ctg"]][,-5]
    }
    unlink(ofn)
    unlink(cfn)
...
.assembleChainBundle <- function(bun, ann) {
    ## prepare
    lgt <- .lgt(ann)
    ...
    acb <- list(ctg=ctgs, aln=alns, chm=chms, fus=fuss)
    return(acb)
}

#' @export
assembleBundle <- function(bun, ann) {
    chain.type <- paste0(bun$type, ".chain")
    bun <- .filterTS(bun, ann)
    buns <- splitBundle(bun, chain.type)
    acbs <- mclapply(names(buns), function(chain) {
        bun <- buns[[chain]]
        acb <- .assembleChainBundle(bun, ann)
        acb <- lapply(acb, function(tbl) tbl[,(chain.type):=as.integer(chain)])
        return(acb)
    })
    if (length(acbs)>0) {
        nasm <- c("ctg", "aln", "chm", "fus")
        asm <- lapply(nasm, function(x) rbindlist(lapply(acbs, "[[", x)))
        names(asm) <- nasm
    } else {
        asm <- ASM.EMPTY
    }
